...
The Golden document picker and the materials sample depend on the omitted local `golden/` collection. Their documents must be supplied privately by an authorized operator; the standard Sites artifact does not include that collection. The generated expenses sample and user-upload flow do not need it. Machine-specific acceptance harnesses and their private evidence are also omitted; `npm test` is the public offline unit-test entry point, with `npm run test:sites` covering the Sites adapters.
