...
export function inspectCalibration(sample,expectedTimeOrigin=null,documentUrl=ORIGIN){
 ...
 const valid=Number.isFinite(before)&&Number.isFinite(after)&&after>=before&&after-before<=100&&Number.isFinite(r?.timeOrigin)&&Number.isFinite(r?.monotonicMs)&&r?.href===documentUrl&&!sample?.renderer?.exceptionDetails&&(expectedTimeOrigin===null||r.timeOrigin===expectedTimeOrigin);
...
export async function collectPreActionCalibration(cdp,raw,{clock=stamp,documentUrl=ORIGIN}={}){
 ...
 raw.clockCalibrationSamples=[];raw.calibrationAdmission={status:'PENDING',documentUrl,started:clock(),selectedOrdinal:null,policy:CALIBRATION_POLICY};let expectedTimeOrigin=null;
 for(let ordinal=1;ordinal<=3;ordinal++){
  ...
  sample.assessment=inspectCalibration(sample,expectedTimeOrigin,documentUrl);
...
export function calibrationAdmissionProblems(raw,{documentUrl=ORIGIN}={}){
 ...
 if(admission?.status!=='READY'||admission.documentUrl!==documentUrl||JSON.stringify(admission.policy)!==JSON.stringify(CALIBRATION_POLICY))problems.push('Calibration admission was not ready under the frozen policy');
 let origin=null,firstValid=null;
 for(let i=0;i<samples.length;i++){const s=samples[i],r=s.renderer?.result?.value;if(origin===null&&Number.isFinite(r?.timeOrigin))origin=r.timeOrigin;const assessed=inspectCalibration(s,origin,documentUrl);if(s.ordinal!==i+1||s.error||JSON.stringify(assessed)!==JSON.stringify(s.assessment))problems.push('Sample order/assessment/error mismatch');if(assessed.valid&&firstValid===null)firstValid=i+1;if(!Number.isFinite(s.started?.monotonicMs)||!Number.isFinite(s.ended?.monotonicMs)||s.ended.monotonicMs<s.started.monotonicMs||(i&&s.started.monotonicMs<samples[i-1].ended.monotonicMs))problems.push('Sample timing is missing/inverted/overlapping');}
