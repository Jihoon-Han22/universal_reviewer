...
export async function loadSample(documents, kind, {root = path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..','golden'), read = readFile} = {}) {
  ...
  try {
    ...
    if (kind === 'expenses') {
      ...
      for (const id of ['P01','P07','P13']) added.push(await documents.add({name:`${id}_시험성적서.pdf`,buffer:await read(path.join(root,'certs',`${id}.pdf`)),role:'target'}));
      added.push(await documents.add({name:'C01_검토기준.xlsx',buffer:await read(path.join(root,'criteria','C01.xlsx')),role:'criteria'}));
