...
export async function loadGolden(documents, request, options = {}) {
  ...
  const added = [];
  try {
    for (const spec of specifications) { const buffer = await readGoldenResource(`${spec.folder}/${spec.id}.${spec.extension}`,options); added.push(await documents.add({name:`${spec.id}_${spec.suffix}.${spec.extension}`,role:spec.role,buffer})); }
